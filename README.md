...
4. Paste in the updated `Code.gs` from this update (it already has the Calendar tab's endpoints).
...
If you'd rather the Calendar tab pull from a separate dedicated calendar instead of that account's main one, swap `CalendarApp.getDefaultCalendar()` for `CalendarApp.getCalendarById('the-other-calendar-id')` in `getScheduleEvents()` in `Code.gs`.
